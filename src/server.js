...
import { getScreener, getTokenDetail, getCandles, getFeed, getHolders, searchTokens, explorerToken, getWallet } from "./stats.js";
...
import { probeAllDomains, probeDomain, syncDiscovered } from "./probe.js";
import { getImage } from "./images.js";
import { gaps, repair } from "./gaps.js";
import { codeGroups } from "./codehash.js";
...
const app = express();
app.use(compression());
// The frontend can be hosted anywhere, so the API answers cross origin reads.
app.use((req, res, next) => { res.set("access-control-allow-origin", "*"); res.set("access-control-allow-methods", "GET,OPTIONS"); if (req.method === "OPTIONS") return res.status(204).end(); next(); }); // screener payloads are mostly repeated JSON keys and shrink by roughly ten times
...
app.get("/api/debug/probe", wrap(async (req, res) => { const d = req.query.domain; res.json(d ? await probeDomain(String(d)) : await probeAllDomains()); }));
app.get("/api/debug/probesync", wrap(async (_, res) => res.json({ updated: await syncDiscovered(state) })));
app.get("/api/debug/sync", wrap(async (_, res) => res.json(await syncLaunchpads(state))));
...
app.get("/api/codegroups", wrap(async (_, res) => res.json(await codeGroups())));
app.get("/api/gaps", wrap(async (_, res) => res.json(await gaps())));
app.get("/api/repair", wrap(async (_, res) => res.json(await repair())));
...
// Logos are served from here rather than hotlinked, so a dead host or a filtered certificate on the
// viewer's machine cannot turn into a missing image.
app.get("/img/:addr", wrap(async (req, res) => {
  if (!ADDR.test(req.params.addr)) return res.status(400).end();
  const img = await getImage(req.params.addr.toLowerCase());
  if (!img) return res.status(404).end();
  res.set("content-type", img.ctype);
...
app.get("/api/wallet/:addr", wrap(async (req, res) => {
  if (!ADDR.test(req.params.addr)) return res.status(400).json({ error: "bad address" });
  res.json(await getWallet(req.params.addr));
}));
...
app.get("/wallet/:addr", (_, res) => { res.set("cache-control", "no-cache"); res.sendFile(path.join(pub, "wallet.html")); });
