...
import { syncLaunchpads } from "./sync.js";
import { probeAllDomains, syncDiscovered } from "./probe.js";
import { cacheImages } from "./images.js";
import { repair } from "./gaps.js";
import { fillCodeHashes, spreadLaunchpadByCode } from "./codehash.js";
import { harvestMeta } from "./harvest.js";
import { findCreationBlock } from "./birth.js";
import { recordVenueTrades } from "./venues.js";
...
import { TOPIC_TRANSFER, TOPIC_POOL_CREATED, TOPIC_SWAP, TOPIC_PAIR_CREATED, TOPIC_SWAP_V2, CHUNK_START, CHUNK_MIN, CHUNK_MAX,
...
async function fetchLogos() {
  const rows = await query(`SELECT t.address, t.birth_block, MAX(p.created_block) AS hint FROM tokens t LEFT JOIN pools p ON p.token = t.address
    WHERE t.meta_checked IS NULL OR (COALESCE(t.logo, '') = '' AND t.meta_checked < $1)
  ...
  for (const { address, hint, birth_block } of rows) {
    ...
    // Only fill blanks. A source that found nothing must never erase what another source found.
...
// Trades that left no Swap event. The USDC transfers point at the transactions worth looking at, then
// the token side of each of those transactions is fetched so the pair can be matched.
async function indexVenues(usdcTransfers, from, to) {
  if (!usdcTransfers.length) return 0;
  const txs = new Set(usdcTransfers.map((l) => l.transactionHash));
  if (!txs.size) return 0;
  let all = [];
  try { all = await getLogsFull({ topics: [TOPIC_TRANSFER] }, from, to); } catch { return 0; }
  const relevant = all.filter((l) => txs.has(l.transactionHash));
  try { return await recordVenueTrades(state, relevant, estimateTs); }
  catch (e) { console.log("[venues]", e.message); return 0; }
}
...
async function indexRange(from, to) {
  ...
  const [created, created2, swapLogs, usdcTransfers] = await Promise.all([
    ...
    // Every USDC movement in the range. Paired with the token transfer in the same transaction this
    // reveals trades on venues that emit no Swap event at all, such as launchpad bonding curves.
    getLogsFull({ address: USDC, topics: [TOPIC_TRANSFER] }, from, to).catch(() => []),
  ]);
  const newPools = [];
  ...
  await indexVenues(usdcTransfers, from, to);
...
export async function start() {
  ...
  setTimeout(() => syncLaunchpads(state).catch((e) => console.log("[sync]", e.message)), 25_000);
  setInterval(() => syncLaunchpads(state).catch(() => {}), 1800_000);
  setTimeout(() => probeAllDomains().then(() => syncDiscovered(state)).catch((e) => console.log("[probe]", e.message)), 60_000);
  setInterval(() => syncDiscovered(state).catch(() => {}), 1200_000);
  setInterval(() => cacheImages().catch((e) => console.log("[images]", e.message)), 45_000);
  setInterval(() => repair().catch(() => {}), 3600_000);
  setInterval(() => fillCodeHashes(state).then(spreadLaunchpadByCode).catch((e) => console.log("[code]", e.message)), 40_000);
  setInterval(() => probeAllDomains().catch(() => {}), 6 * 3600_000);
