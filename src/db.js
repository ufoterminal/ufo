...
export async function init() {
  await q(`
    ...
    ALTER TABLE tokens ADD COLUMN IF NOT EXISTS code_hash TEXT;
    CREATE INDEX IF NOT EXISTS tokens_code_hash ON tokens(code_hash);
    ...
    CREATE TABLE IF NOT EXISTS image_patterns (pattern TEXT PRIMARY KEY, hits INTEGER NOT NULL DEFAULT 0, last_hit BIGINT);
    CREATE TABLE IF NOT EXISTS token_images (address TEXT PRIMARY KEY, ctype TEXT NOT NULL, bytes BYTEA NOT NULL, src TEXT NOT NULL DEFAULT '', fetched BIGINT);
    CREATE TABLE IF NOT EXISTS lp_endpoints (domain TEXT PRIMARY KEY, label TEXT NOT NULL DEFAULT '', host TEXT NOT NULL, path TEXT NOT NULL, matched INTEGER NOT NULL DEFAULT 0, checked BIGINT);
